    ...
    { 
     stage('Pull') {
      ...
      stage('Build') {
            ...
            sh(script: '''sudo docker build -f -t /var/lib/jenkins/workspace/OurCar_django/final_project/Dockerfile ourcar_test .''')
        }
      ...
      stage('Deploy') {
            sshagent(credentials: ['AWS_EC2_Ourcar_Server']) {
                ...
                sh(script: 'ssh ubuntu@3.34.74.38 "sudo docker run --name ourcar_test -d -v /home/ubuntu/my_settings.py:/app/.env -e TZ=Asia/Seoul -p 8000:8000 \${DOCKER_USER_ID}/ourcar_test:\${BUILD_NUMBER}"')
        }
    }
