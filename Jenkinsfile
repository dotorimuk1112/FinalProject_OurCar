     ...
     stage('Pull') {
      ...
      stage('Deploy') {
            sshagent(credentials: ['AWS_EC2_Ourcar_Server']) {
                ...
                sh(script: 'ssh ubuntu@3.34.74.38 "sudo docker run --name ourcar_test -d -v /home/ubuntu/.env:/app/.env -v /home/ubuntu/ai_models:/app/final_project/ai_models -e TZ=Asia/Seoul ${DOCKER_USER_ID}/ourcar_test:${BUILD_NUMBER}"')
        }
    }
