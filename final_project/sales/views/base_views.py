...
def my_page(request):
    ...
    liked_questions = CarSalesPost.objects.filter(buyer=user)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return render(request, 'sales/my_page.html', {'liked_questions': page_obj, 'test_sangmin_instance': test_sangmin_instance, 'profile_image': profile_image, 'profile_image_form': form})  # Render the user's profile page
            # return redirect("/")  # Render the user's profile page
    else:
        form = ProfileImageForm(instance=request.user)
        print("get----------------------")

    return render(request, 'sales/my_page.html', {'liked_questions': page_obj, 'test_sangmin_instance': test_sangmin_instance, 'profile_image': profile_image, 'profile_image_form': form})
